...
typedef struct {
  ...
  /* Output link for entry batch bytes sent to the dexf tile
     (Firedancer-only, optional). */
  ulong       dexf_out_idx;
  fd_wksp_t * dexf_out_mem;
  ulong       dexf_out_chunk0;
  ulong       dexf_out_wmark;
  ulong       dexf_out_chunk;
  ...
  struct {
    ...
    ulong fec_sets_completed; /* FEC sets forwarded to dexf tile */
  } metrics[ 1 ];
...
after_frag( fd_shred_ctx_t *    ctx,
  ...
  for( ulong fset_k=0; fset_k<ctx->send_fec_set_cnt; fset_k++ ) {
    ...
    if( FD_LIKELY( ctx->store ) ) { /* firedancer-only */
      ...
      /* Publish entry batch bytes to the dexf tile if connected */
      if( FD_UNLIKELY( ctx->dexf_out_idx!=ULONG_MAX && fec->data_sz>0UL ) ) {
        ulong txp_sig = (last->slot << 32UL) | (ulong)last->fec_set_idx;
        uchar * dst = fd_chunk_to_laddr( ctx->dexf_out_mem, ctx->dexf_out_chunk );
        fd_memcpy( dst, fec->data, fec->data_sz );
        ulong tspub = fd_frag_meta_ts_comp( fd_tickcount() );
        fd_stem_publish( stem, ctx->dexf_out_idx, txp_sig, ctx->dexf_out_chunk, fec->data_sz, 0UL, ctx->tsorig, tspub );
          ctx->dexf_out_chunk = fd_dcache_compact_next( ctx->dexf_out_chunk, fec->data_sz, ctx->dexf_out_chunk0, ctx->dexf_out_wmark );
    ...
    /* Publish entry batch bytes to the dexf tile (Frankendancer path).
    ...
    if( FD_UNLIKELY( !ctx->store && ctx->dexf_out_idx!=ULONG_MAX ) ) {
      uchar * dst     = fd_chunk_to_laddr( ctx->dexf_out_mem, ctx->dexf_out_chunk );
      ulong   data_sz = 0UL;
      for( ulong i=0UL; i<set->data_shred_cnt; i++ ) {
        fd_shred_t * data_shred = (fd_shred_t *)fd_type_pun( set->data_shreds[i] );
      ...
      if( FD_LIKELY( data_sz>0UL ) ) {
        ulong txp_sig = (last->slot << 32UL) | (ulong)last->fec_set_idx;
        ulong tspub   = fd_frag_meta_ts_comp( fd_tickcount() );
        fd_stem_publish( stem, ctx->dexf_out_idx, txp_sig, ctx->dexf_out_chunk, data_sz, 0UL, ctx->tsorig, tspub );
        ctx->dexf_out_chunk = fd_dcache_compact_next( ctx->dexf_out_chunk, data_sz, ctx->dexf_out_chunk0, ctx->dexf_out_wmark );
        ctx->metrics->fec_sets_completed++;
      }
    }
...
unprivileged_init( fd_topo_t *      topo,
  ...
  ctx->dexf_out_idx  = fd_topo_find_tile_out_link( topo, tile, "shred_dexf", ctx->round_robin_id );
  if( FD_UNLIKELY( ctx->dexf_out_idx!=ULONG_MAX ) ) { /* firedancer-only, optional */
    fd_topo_link_t * dexf_out = &topo->links[ tile->out_link_id[ ctx->dexf_out_idx ] ];
    ctx->dexf_out_mem    = topo->workspaces[ topo->objs[ dexf_out->dcache_obj_id ].wksp_id ].wksp;
    ctx->dexf_out_chunk0 = fd_dcache_compact_chunk0( ctx->dexf_out_mem, dexf_out->dcache );
    ctx->dexf_out_wmark  = fd_dcache_compact_wmark ( ctx->dexf_out_mem, dexf_out->dcache, dexf_out->mtu );
    ctx->dexf_out_chunk  = ctx->dexf_out_chunk0;
  } else {
    ctx->dexf_out_idx = ULONG_MAX;
  }
