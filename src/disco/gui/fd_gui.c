...
fd_gui_network_stats_snap( fd_gui_t *               gui,
  ...
  if( FD_LIKELY( smcast_eg_idx!=ULONG_MAX ) ) {
    ...
    cur->in.sig_failed     = sm_eg_met[ FD_METRICS_COUNTER_SHRED_MCAST_SIG_FAILED_OFF      ];
  }
    ...
    for( ulong i=0UL; i<src_cnt; i++ ) {
      ...
      if( FD_LIKELY( sender_ip != 0U ) ) {
        fd_cstr_printf( cur->mcast_src_label[ i ], 24UL, NULL, "%u.%u.%u.%u",
                        sender_ip & 0xFFU, (sender_ip>>8) & 0xFFU, (sender_ip>>16) & 0xFFU, (sender_ip>>24) & 0xFFU );
      } else {
