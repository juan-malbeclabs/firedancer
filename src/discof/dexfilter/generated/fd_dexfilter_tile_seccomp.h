...
#ifndef HEADER_fd_src_discof_dexfilter_generated_fd_dexfilter_tile_seccomp_h
#define HEADER_fd_src_discof_dexfilter_generated_fd_dexfilter_tile_seccomp_h
...
static const unsigned int sock_filter_policy_fd_dexfilter_tile_instr_cnt = 18;

static void populate_sock_filter_policy_fd_dexfilter_tile( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd, uint dexfilter_fd, uint swaplog_fd ) {
  FD_TEST( out_cnt >= 18 );
...
//  lbl_2:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, dexfilter_fd, /* RET_ALLOW */ 5, /* lbl_3 */ 0 ),
//  lbl_3:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, swaplog_fd, /* RET_ALLOW */ 3, /* RET_KILL_PROCESS */ 2 ),
//  check_fsync:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
...
#endif /* HEADER_fd_src_discof_dexfilter_generated_fd_dexfilter_tile_seccomp_h */
